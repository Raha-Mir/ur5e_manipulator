...
class UR5eHighAccSpiralCartesian:
    def __init__(self):
        ...
        # Spiral settings
        self.center = np.array([0.4, 0.0, 0.3])  # Start position
        self.radius = 0.1
        self.period = 6.0                 # Time for one full circle
        self.spiral_rise = 0.05            # Height increase per revolution (m)
        self.spiral_loops = 3              # Number of spiral loops to complete
        ...
        # Initialize q_pre_target using IK for spiral start position
        R_target = np.diag([1, -1, -1])
        spiral_start = np.array([self.center[0] + self.radius, self.center[1], self.center[2]])
        self.q_pre_target = self.compute_ik(spiral_start, R_target)
    ...
    def compute_desired_pose(self, t):
        ...
        spiral_angle = omega * t
        revolutions = t / self.period
        
        # Spiral motion in X-Z plane with vertical rise
        x = self.center[0] + self.radius * np.cos(spiral_angle)
        z = self.center[2] + self.radius * np.sin(spiral_angle) + (revolutions * self.spiral_rise)
        
        # Keep y fixed at center (or could add lateral movement too)
        y = self.center[1] 
        
        pos = np.array([x, y, z])
        R = np.diag([1, -1, -1])  # Maintain same orientation
        return pos, R
    ...
    def joint_state_callback(self, msg):
        ...
        if not self.pre_motion_done:
            ...
            if s >= 1.0:
                ...
                rospy.loginfo("Pre-motion phase done. Starting spiral motion.")
                t_elapsed = 0.0
        else:
            # Calculate elapsed time for spiral motion
            t_elapsed = (now - self.start_time).to_sec()
            
            # Stop after completing specified loops
            if t_elapsed > self.spiral_loops * self.period:
                rospy.loginfo("Spiral motion completed. Shutting down.")
                rospy.signal_shutdown("Spiral motion completed")
                return
        ...
        rospy.loginfo_throttle(1.0, f"t={t_elapsed:.2f} pre_motion={not self.pre_motion_done} z={actual_pos[2]:.3f}")
    ...
    def on_shutdown(self):
        ...
        log_path = log_dir / 'ur5e_spiral_cartesian.pkl'

        with open(log_path, 'wb') as f:
            pickle.dump(data, f)
        rospy.loginfo(f"Saved spiral Cartesian trajectory logs to {log_path}")

if __name__ == '__main__':
    rospy.init_node("ur5e_highacc_spiral_cartesian")
    rospy.loginfo("Starting UR5e Cartesian Spiral Trajectory Controller with Pre-Motion...")
    controller = UR5eHighAccSpiralCartesian()
    rospy.spin()
